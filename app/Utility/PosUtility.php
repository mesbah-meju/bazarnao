...
class PosUtility
{
    public static function product_search($request_data): object
    {
        ...
        if ($request_data['category'] != null) {
            ...
            if ($arr[0] == 'category') {
                $category = Category::find($arr[1]);
                $product_query = $category->products();
            }
        }

        $product_query->join('product_stocks', 'product_stocks.product_id', '=', 'products.id');
        $products = $product_query->where('products.published', 1)
