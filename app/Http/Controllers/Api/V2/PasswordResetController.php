...
use App\Notifications\EmailVerificationNotificationCode;
...
class PasswordResetController extends Controller
{
    public function forgetRequest(Request $request)
    {
        if ($request->send_code_by == 'email') {
        	...
        	if(empty($user)){
            	$user = User::where('phone', str_replace("+88","",$request->email_or_phone))->first();
            }
        }
    ...
    public function confirmReset(Request $request)
    {
        ...
        if ($user != null) {
            ...
            return response()->json([
                'result' => false,
                'message' => 'Wong Code !! You have provided wong code.',
            ], 200);
        }
    }
