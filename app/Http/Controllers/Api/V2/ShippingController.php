...
class ShippingController extends Controller
{
    public function shipping_cost(Request $request)
    {
        ...
        $calculate_total+=($product->unit_price*$shop_items_raw_data_item->quantity);
    ...
    if($shipping_skip_total<=$calculate_total){
        ...
        return response()->json(['result' => true, 'shipping_type' => get_setting('shipping_type'), 'value' => convert_price($total_shipping_cost) ,'value_string'=>format_price($total_shipping_cost),'total'=>format_price($calculate_total)], 200);
    }else{
		$total_shipping_cost = BusinessSetting::where('type', 'flat_rate_shipping_cost')->first()->value;
        return response()->json(['result' => true, 'shipping_type' => get_setting('shipping_type'), 'value' => convert_price($total_shipping_cost) ,'value_string'=>format_price($total_shipping_cost),'total'=>format_price($calculate_total)], 200);
	}
        $shop_items_raw_data = Cart::where('user_id', $request->user_id)->where('owner_id', $request->owner_id)->get();
        foreach ($shop_items_raw_data as $key => $shop_items_raw_data_item) {
            ...
            if (get_setting('shipping_type') == 'flat_rate') {
                ...
                if ($product->added_by == 'admin') {
                    $value = BusinessSetting::where('type', 'shipping_cost_admin')->first()->value / count($admin_products);
                } else {
                    $value = Shop::where('user_id', $product->user_id)->first()->shipping_cost / count($seller_products[$product->user_id]);
                }
            } elseif (get_setting('shipping_type') == 'area_wise_shipping') {
                if ($product->added_by == 'admin') {
