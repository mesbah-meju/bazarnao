...
class ReportController extends Controller
{
    ...
    // {
    //     ini_set('max_execution_time', 0); // Disable time limits for large reports
    ...
    //         $productsQuery = Product::select('products.*', 'categories.id as cat_id', 'categories.name as cat_name')
    //             ->leftJoin('categories', 'products.category_id', 'categories.id')
    //             ->whereNull('products.parent_id');

    //         if ($request->category_id) {
    //             $productsQuery->where('categories.id', $category_id);
    //         }

    //         if ($request->product_id) {
    //             $productsQuery->where('products.id', $product_id);
    //         }

    //         $products = $productsQuery->get();
    //         $product_ids = $products->pluck('id')->unique();
    ...
    //         // $purchases = PurchaseDetail::select('purchase_details.id', 'purchase_details.product_id', 'purchase_details.wearhouse_id', 'purchase_details.qty', 'purchase_details.price', 'purchase_details.amount', 'purchases.status', 'purchases.date')
    //         //     ->leftjoin('purchases', 'purchases.id', '=', 'purchase_details.purchase_id')
    //         //     ->where('purchases.status', 2)
    //         //     ->whereIn('purchase_details.product_id', $product_ids)
    ...
    //         $main_refunds = RefundRequest::select('refund_requests.id', 'order_details.product_id', 'refund_requests.return_qty as qty', 'refund_requests.return_amount as amount', 'refund_requests.purchase_price as price')
    ...
    //         $child_refunds = RefundRequest::select(
    //             'refund_requests.id',
    //             'products.parent_id as product_id',
    //             DB::raw('products.deduct_qty * refund_requests.return_qty as qty'),
    //             'refund_requests.return_amount as amount',
    //             'refund_requests.purchase_price as price'
    //         )
    //             ->leftJoin('order_details', 'order_details.id', '=', 'refund_requests.order_detail_id')
    //             ->leftJoin('orders', 'orders.id', '=', 'order_details.order_id')
    //             ->leftJoin('products', 'products.id', '=', 'order_details.product_id')
    //             ->whereIn('products.parent_id', $product_ids)
    //             ->where('refund_requests.refund_status', 5)
    //             ->where('orders.warehouse', $warehouse_id)
    //             ->whereBetween('refund_requests.created_at', [$from_date, $to_date])
    //             ->get();
    ...
    //         $refunds = $main_refunds->merge($child_refunds)->groupBy('product_id');
    ...
    //         $child_orders = OrderDetail::select('order_details.id', 'order_details.order_id', 'products.parent_id as product_id', 'order_details.price', DB::raw('products.deduct_qty * order_details.quantity as qty'), 'order_details.delivery_status', 'orders.warehouse', 'order_details.created_at', 'order_details.updated_at')
    //             ->leftJoin('products', 'products.id', '=', 'order_details.product_id')
    //             ->leftJoin('orders', 'orders.id', '=', 'order_details.order_id')
    //             ->where('order_details.delivery_status', 'delivered')
    //             ->whereIn('products.parent_id', $product_ids)
    ...
    //         // Loop through each product and aggregate the stock data
    //         foreach ($products as $product) {
    //             $product->opening_stock_qty = 0;
    //             $product->opening_stock_amount = 0;
    //             $product->purchase_qty = 0;
    //             $product->purchase_amount = 0;
    //             $product->receive_qty = 0;
    //             $product->receive_amount = 0;
    //             $product->sales_qty = 0;
    //             $product->sales_amount = 0;
    //             $product->refund_qty = 0;
    //             $product->refund_amount = 0;
    //             $product->damage_qty = 0;
    //             $product->damage_amount = 0;
    //             $product->transfer_qty = 0;
    //             $product->transfer_amount = 0;
    //             $product->closing_stock_qty = 0;
    //             $product->closing_stock_amount = 0;

    //             // Merge inflow stock: opening stocks, purchases, received, refunds
    //             $purchase_items = collect();
    //             if (isset($opening_stocks[$product->id])) {
    //                 foreach ($opening_stocks[$product->id] as $stock) {
    //                     $product->opening_stock_qty += $stock->qty;
    //                     $product->opening_stock_amount += $stock->qty * $stock->price;
    //                 }

    //                 $purchase_items = $purchase_items->merge($opening_stocks[$product->id]);
    //             }

    //             if (isset($purchases[$product->id])) {
    //                 foreach ($purchases[$product->id] as $purchase) {
    //                     $product->purchase_qty += $purchase->qty;
    //                     $product->purchase_amount += $purchase->qty * $purchase->price;
    //                 }

    //                 $purchase_items = $purchase_items->merge($purchases[$product->id]);
    //             }

    //             if (isset($receives[$product->id])) {
    //                 foreach ($receives[$product->id] as $receive) {
    //                     $product->receive_qty += $receive->qty;
    //                     $product->receive_amount += $receive->qty * $receive->price;
    //                 }

    //                 $purchase_items = $purchase_items->merge($receives[$product->id]);
    //             }

    //             if (isset($refunds[$product->id])) {
    //                 foreach ($refunds[$product->id] as $refund) {
    //                     $product->refund_qty += $refund->qty;
    //                     $product->refund_amount += $refund->amount;
    //                 }

    //                 $purchase_items = $purchase_items->merge($refunds[$product->id]);
    //             }


    //             // Merge outflow stock: sales, transfers, damages
    //             $sales_items = collect();

    //             if (isset($sales[$product->id])) {
    //                 foreach ($sales[$product->id] as $sale) {
    //                     $product->sales_qty += $sale->qty;
    //                     $product->sales_amount += $sale->price;
    //                 }

    //                 $sales_items = $sales_items->merge($sales[$product->id]);
    //             }

    //             if (isset($damages[$product->id])) {
    //                 foreach ($damages[$product->id] as $damage) {
    //                     $product->damage_qty += $damage->qty;
    //                     $product->damage_amount += $damage->amount;
    //                 }

    //                 $sales_items = $sales_items->merge($damages[$product->id]);
    //             }

    //             if (isset($transfers[$product->id])) {
    //                 foreach ($transfers[$product->id] as $transfer) {
    //                     $product->transfer_qty += $transfer->qty;
    //                     $product->transfer_amount += $transfer->qty * $transfer->price;
    //                 }

    //                 $sales_items = $sales_items->merge($transfers[$product->id]);
    //             }

    //             // FIFO Calculation and closing stock
    //             $remaining_sales_qty = $product->sales_qty + $product->transfer_qty + $product->damage_qty;

    //             // Sort inflows by date for FIFO
    //             $purchase_items = $purchase_items->sortBy('date');

    //             $fifo_array = array();
    //             foreach ($purchase_items as $purchase) {
    //                 if ($remaining_sales_qty > 0) {
    //                     if ($purchase->qty >= $remaining_sales_qty) {
    //                         $fifo_array['product_id'] = $product->id;
    //                         $fifo_array['product_id'] = $product->id;

    //                         // Decrease the stock and account for the sale
    //                         $purchase->qty -= $remaining_sales_qty;
    //                         $remaining_sales_qty = 0; // All sales have been accounted for
    //                     } else {
    //                         // Reduce the remaining sales quantity by the amount available in stock
    //                         $remaining_sales_qty -= $purchase->qty;
    //                         $purchase->qty = 0; // Deplete this purchase item completely
    //                     }
    //                 }

    //                 // If any stock remains after the sales, add it to closing stock
    //                 if ($purchase->qty > 0) {
    //                     $product->closing_stock_qty += $purchase->qty;
    //                     $product->closing_stock_amount += ($purchase->qty * $purchase->price);
    //                 }
    //             }

    //             // Ensure non-negative closing quantities
    //             if ($product->closing_stock_qty < 0) {
    //                 $product->closing_stock_qty = 0;
    //                 $product->closing_stock_amount = 0;
    //             }
    //             // $product_stock =  ProductStock::where('product_id',$product->id)->where('wearhouse_id',$warehouse_id)->first();
    //             // $product_stock->qty = $product->closing_stock_qty;
    //             // $product_stock->opening_stock = $product->opening_stock_qty;
    //             // $product_stock->save();
    ...
    //                     'wearhouse_id' => $warehouse_id,
    //                     'qty' => $product->closing_stock_qty,
    //                     'opening_stock' => $product->opening_stock_qty,
    //                 ]);
    //             }
    //         }
    //     } else {
    ...
    public function monthly_stock_ledger_report(Request $request) // Added by Tanem
    {
        ...
        if ($request->has('warehouse_id') && !empty($request->warehouse_id)) {
            ...
            foreach ($products as $product) {
                ...
                // dd($product);
                // Update the stock quantities for each product
                // ✅ Update ProductStock only for current month's report
    ...
    public function product_wise_fifo_report(Request $request)
    {
        ini_set('max_execution_time', 0); // Disable time limits for large reports
        ...
        if ($request->has('warehouse_id') && !empty($request->warehouse_id)) {
            ...
            // $purchases = Purchase_order_item::select('purchase_order_item.id', 'purchase_order_item.product_id', 'purchase_order_item.wearhouse_id', 'purchase_order_item.qty', 'purchase_order_item.price', 'purchase_order_item.amount', 'purchase_order.status', 'purchase_order.date')
            ...
            $refunds = RefundRequest::select('refund_requests.id', 'order_details.product_id', 'refund_requests.return_qty as qty', 'refund_requests.return_amount as amount')
                ->leftJoin('order_details', 'order_details.id', '=', 'refund_requests.order_detail_id')
                ->leftJoin('orders', 'orders.id', '=', 'order_details.order_id')
            ...
            // Add consolidated row for opening stock
            $transactions = array();
            if (isset($opening_stocks[$product_id])) {
                ...
                foreach ($opening_stocks[$product_id] as $stock) {
                    $totalOpeningQty += $stock->qty;
                    $totalOpeningAmount += $stock->qty * $stock->price;
                    if (!$openingDate) {
                        $openingDate = $stock->created_at;
                    }
                }
                
                // Create single consolidated opening stock row
                if ($totalOpeningQty > 0 || $totalOpeningAmount > 0) {
                    $transactions[] = [
                        'id'            => 'opening_stock',
                        'particulars'   => 'Opening Stock',
                        'date'          => date('Y-m-d H:i:s', strtotime($openingDate)),
                        'qty'           => $totalOpeningQty,
                        'per_unit'      => $totalOpeningQty > 0 ? ($totalOpeningAmount / $totalOpeningQty) : 0,
            ...
            // Step 1: Sort $transactions by date in ascending order
            ...
            foreach ($transactions as $transaction) {
                ...
                if ($type === 'Receipt') {
                    // Add receipt to the balance stack
                    $balance[] = ['qty' => $qty, 'per_unit' => $perUnit];
                    $receiptQty = $qty;
                    $receiptPerUnit = $perUnit;
                    $receiptTotal = $total;
                } elseif ($type === 'Issue') {
                    // Handle FIFO for issue with batch detail
                    $issueQty = $qty;
                    $remainingQty = $qty;
                    $issueTotal = 0;
                    ...
                    while ($remainingQty > 0 && !empty($balance)) {
                        ...
                        if ($batch['qty'] <= $remainingQty) {
                            ...
                            array_unshift($balance, $batch); // Push updated batch back to balance
                            $remainingQty = 0;
                        }
                    }
                ...
                // Calculate current balance by batching different unit prices
                $balanceDetails = [];
                ...
                $inventoryData[] = [
                    ...
                    'issueQty' => $issueQty,
                    'issueDetails' => $issueDetails, // Detailed issue breakdown
                    'issueTotal' => $issueTotal,
                    'balanceDetails' => $balanceDetails, // Multiple entries for balance with different unit prices
                ];
            }

            // Sum up total balance quantities and values for the footer
            $totalBalanceQty = array_sum(array_column($balance, 'qty'));
            $totalBalanceValue = array_reduce($balance, fn($sum, $item) => $sum + ($item['qty'] * $item['per_unit']), 0);
        } else {
            $products = null;
        }

        return view('backend.reports.single_product_fifo_transections', compact('inventoryData', 'totalBalanceQty', 'totalBalanceValue', 'totalIssueQty', 'costOfGoodsSold', 'warehouse_id', 'category_id', 'product_id', 'from_date', 'to_date'));
    }
    ...
    //                 if ($remaining_sales_qty > 0) {
    //                     if ($purchase->qty >= $remaining_sales_qty) {
    //                         $product->closing_stock_qty += ($purchase->qty - $remaining_sales_qty);
    //                         $product->closing_stock_amount += ($purchase->qty - $remaining_sales_qty) * $purchase->price;
    ...
    public function stock_closing(Request $request) // Added by Tanem
    {
        ini_set('max_execution_time', 0); // Disable time limits for large reports
        ...
        if ($request->has('warehouse_id') && !empty($request->warehouse_id)) {
            ...
            $child_orders = OrderDetail::select('order_details.id', 'order_details.order_id', 'products.parent_id as product_id', 'order_details.price', DB::raw('products.deduct_qty * order_details.quantity as qty'), 'order_details.delivery_status', 'orders.warehouse', 'order_details.created_at', 'order_details.updated_at')
                ...
                ->leftJoin('orders', 'orders.id', '=', 'order_details.order_id')
                ->where('order_details.delivery_status', 'delivered')
                ->whereIn('products.parent_id', $product_ids) // Ensure you're getting child products
                ->where('orders.warehouse', $warehouse_id)
                ->whereBetween('orders.delivered_date', [$from_date, $to_date])
                ->get();
            ...
            foreach ($products as $product) {
                ...
                // Merge outflow stock: sales, damages, transfers
                $sales_items = collect();
                if (isset($sales[$product->id])) {
                    foreach ($sales[$product->id] as $sale) {
                        $product->sales_qty += $sale->qty;
                ...
                // Calculate remaining quantities after sales and transfers for FIFO
                $remaining_sales_qty = $product->sales_qty + $product->transfer_qty + $product->damage_qty;
                ...
                foreach ($purchase_items as $purchase) {
                    if ($remaining_sales_qty > 0) {
                        ...
                        // Add remaining stock to closing quantities
                        $product->closing_stock_qty += $purchase->qty;
                        $product->closing_stock_amount += $purchase->qty * $purchase->price;
                    }
                }

                // Add refund amounts to closing stock amount
                // $product->closing_stock_amount = $product->opening_stock_amount + $product->purchase_amount + $product->receive_amount - $product->sales_amount + $product->refund_amount - $product->damage_amount - $product->transfer_amount;
        ...
        // Return the stock closing report view
        return view('backend.reports.stock_closing', compact('products', 'warehouse_id', 'category_id', 'product_id', 'month'));
    }
    ...
    public function save_stock_closing(Request $request) // Added by Tanem
    {
        ini_set('max_execution_time', 0);
        ...
        try {
            ...
            if ($request->has('warehouse_id') && !empty($request->warehouse_id)) {
                ...
                $child_refunds = RefundRequest::select('refund_requests.id', 'products.parent_id as product_id', DB::raw('products.deduct_qty * refund_requests.return_qty as qty'), 'refund_requests.return_amount as amount', 'refund_requests.purchase_price as price')
                    ->leftJoin('order_details', 'order_details.id', '=', 'refund_requests.order_detail_id')
                    ->leftJoin('orders', 'orders.id', '=', 'order_details.order_id')
                    ->leftJoin('products', 'products.id', '=', 'order_details.product_id')
                    ->whereIn('products.parent_id', $product_ids)
                ...
                OpeningStock::where('wearhouse_id', $warehouse_id)
                    ->whereBetween('created_at', array($nextMFdate, $nextMLdate))
                    ->delete();
                ...
                foreach ($products as $product) {
                    ...
                    foreach ($purchase_items as $purchase) {
                        ...
                        $existing_row = OpeningStock::select('id', 'qty')->where('product_id', $product->id)
                            ->where('wearhouse_id', $warehouse_id)
            ...
            flash(translate('Stock closing failed. Please try again. Error: ') . $e->getMessage())->error();
    ...
    public function customer_ledger(Request $request)
    {
        if (!empty($request->user_id) && !empty($request->warehouse)) {
            ...
            if (!$canSeeAllWarehouses) {
                ...
                $warehouseIds = implode(',', $warehousearray);
                $sql2 .= " AND orders.warehouse IN ($warehouseIds)";
            }
    ...
    // public function supplier_ledger_last(Request $request)
    // {
    //     $wearhouse = $request->has('warehouse') && !empty($request->warehouse)
    //         ? $request->warehouse
    //         : implode('', getWearhouseBuUserId(Auth::user()->id));

    //     // Date Filter Initialization
    //     $start_date = date('Y-m-01'); // Default: first day of this month
    //     $end_date = date('Y-m-t');    // Default: last day of this month
    //     $month = null;
    //     $year = null;
    //     $month_year = null;
    ...
    //         (
    //             SELECT SUM(sll.debit - sll.credit)
    //             FROM supplier_ledger AS sll
    //             WHERE s.supplier_id = sll.supplier_id
    //             AND sll.date < '{$start_date}'
    //         ) AS opening_balance
    //         FROM suppliers s
    //         LEFT JOIN supplier_ledger sl ON s.supplier_id = sl.supplier_id
    //         LEFT JOIN purchases po ON sl.purchase_id = po.id
    //         WHERE (sl.date BETWEEN '{$start_date}' AND '{$end_date}' OR sl.date IS NULL)
    //         AND po.wearhouse_id = {$wearhouse}
    //         AND sl.debit != 0 AND sl.credit != 0
    //         GROUP BY s.supplier_id
    //         ORDER BY s.name ASC";
    ...
    //                 FROM supplier_ledger AS sll
    //                 WHERE sll.supplier_id = {$c->supplier_id}
    //                 AND sll.date < '{$start_date}'";
    //         } else {
    //             $opening_sql = "SELECT SUM(sll.debit - sll.credit) AS opening_balance
    //                 FROM supplier_ledger AS sll
    //                 LEFT JOIN purchases po ON sll.purchase_id = po.id
    //                 WHERE po.wearhouse_id = {$wearhouse}
    //                 AND sll.supplier_id = {$c->supplier_id}
    //                 AND sll.date < '{$start_date}'";
    //         }

    //         $opening_balance_info = DB::select($opening_sql);
    //         $customers[$key]->opening_balance = $opening_balance_info[0]->opening_balance ?? 0;
    ...
    //         $debit_credit_balance_info = DB::select($dbcrb_sql);
    //         $customers[$key]->debit = $debit_credit_balance_info[0]->debit ?? 0;
    //         $customers[$key]->credit = $debit_credit_balance_info[0]->credit ?? 0;
    //         $customers[$key]->balance = $debit_credit_balance_info[0]->balance ?? 0;
    //     }

    //     return view('backend.reports.supplier_ledger_new', compact(
    //         'customers', 'start_date', 'end_date', 'wearhouse', 'month', 'year', 'month_year'
    //     ));
    // }
    ...
    //     $wearhouse = $request->has('warehouse') && !empty($request->warehouse)
    //         ? $request->warehouse
    //         : implode(',', getWearhouseBuUserId(Auth::user()->id)); // Fixed implode (was missing ',')
    ...
    //     LEFT JOIN purchases po ON sl.purchase_id = po.id
    //     WHERE (sl.date BETWEEN '{$start_date}' AND '{$end_date}' OR sl.date IS NULL)
    //     AND po.wearhouse_id IN ({$wearhouse})
    //     AND sl.debit != 0 AND sl.credit != 0
    //     GROUP BY s.supplier_id
    //     ORDER BY s.name ASC"; // Fixed missing IN when multiple warehouses are present

    //     $customers = DB::select('SELECT * FROM suppliers');

    //     foreach ($customers as $key => $c) {
    //         if (empty($request->warehouse)) { // use $request->warehouse, not $wearhouse for checking
    //             $opening_sql = "SELECT SUM(sll.debit - sll.credit) AS opening_balance
    //             FROM supplier_ledger AS sll
    //             WHERE sll.supplier_id = {$c->supplier_id}
    //             AND sll.date < '{$start_date}'";
    ...
    //         if (empty($request->warehouse)) { // same here: check $request->warehouse
    //             $dbcrb_sql = "SELECT SUM(sll.debit) AS debit, SUM(sll.credit) AS credit, SUM(sll.balance) AS balance
    //             FROM supplier_ledger AS sll
    //             WHERE sll.supplier_id = {$c->supplier_id}
    //             AND sll.date BETWEEN '{$start_date}' AND '{$end_date}'";
    ...
    public function supplier_ledger(Request $request)
    {
        ...
        if ($supplier_id) {
            $suppliersQuery->where('supplier_id', $supplier_id);
        }
        $customers = $suppliersQuery->get();
    ...
    public function number_of_invoice(Request $request)
    {
        ...
        if ($warehouse) {
            $order_details = $order_details->where('orders.warehouse', $warehouse);
        }
    ...
    public function product_fifo_detail_report(Request $request)
    {
        ...
        $start_date = !empty($request->start_date) ? date('Y-m-d 00:00:00', strtotime($request->start_date)) : date('Y-m-01 00:00:00');
        $end_date = !empty($request->end_date) ? date('Y-m-d 23:59:59', strtotime($request->end_date)) : date('Y-m-t 23:59:59');
        $product_id = $request->product_id;
        $warehouse = empty($request->warehouse) ? [1, 2, 3] : (array)$request->warehouse;
        ...
        // Fetch Sales, Purchases, Transfers, Damages, Refunds, and Opening Stocks
        ...
        // Initialize arrays for FIFO details and stock tracking
        $fifoDetails = [];
        $stockQueue = [];  // FIFO stock tracking array
        $total_sales = 0;
        $total_costs = 0;

        // Function to add stock entries to FIFO queue
        function addStockToQueue(&$stockQueue, $units, $cost)
        {
            $stockQueue[] = ['units' => $units, 'cost' => $cost];
        }

        // Function to deduct stock from FIFO queue
        function deductStockFromQueue(&$stockQueue, $units)
        {
            $deductedCost = 0;
            while ($units > 0 && !empty($stockQueue)) {
                if ($stockQueue[0]['units'] <= $units) {
                    $deductedCost += $stockQueue[0]['units'] * $stockQueue[0]['cost'];
                    $units -= $stockQueue[0]['units'];
                    array_shift($stockQueue);
                } else {
                    $deductedCost += $units * $stockQueue[0]['cost'];
                    $stockQueue[0]['units'] -= $units;
                    $units = 0;
                }
            }
            return $deductedCost;
        ...
        foreach ($openingStocks as $item) {
            $fifoDetails[] = [
                ...
                'total_sales' => 0,
                'remaining_stock_cost' => implode(", ", array_map(fn($s) => "{$s['units']}->{$s['cost']}", $stockQueue))
            ];
            addStockToQueue($stockQueue, $item->qty, $item->price);
        }
        ...
        foreach ($sales as $item) {
            $cost = deductStockFromQueue($stockQueue, $item->quantity);
            $fifoDetails[] = [
                'date' => $item->created_at->format('Y-m-d'),
                'transaction' => 'Sale',
                'units' => $item->quantity,
        ...
        foreach ($transfersIn as $item) {
            ...
            addStockToQueue($stockQueue, $item->qty, $item->unit_price);
        }

        // Process Transfers Out
        foreach ($transfersOut as $item) {
            $cost = deductStockFromQueue($stockQueue, $item->qty);
            $fifoDetails[] = [
                'date' => $item->date,
        ...
        // dd($fifoDetails);
        // usort($fifoDetails, fn($a, $b) => strtotime($a['date']) - strtotime($b['date']));
        $total_costs = array_sum(array_column($fifoDetails, 'total_costs'));


        return view('backend.reports.product_fifo_details_report', compact('fifoDetails', 'product_name', 'start_date', 'end_date', 'total_sales', 'total_costs'));
    }
    ...
    //             'total_costs' => $item->amount,
    //             'total_sales' => 0,
    //             'remaining_stock_cost' => "{$item->qty}->{$item->price}"
    //         ];
    //         addStockToQueue($stockQueue, $item->qty, $item->price);
    //     }
    ...
    //         ];
    //         addStockToQueue($stockQueue, $item->qty, $item->unit_price);
    //         $total_transferin += $item->qty * $item->unit_price;
    //     }
    ...
    //     //         'total_sales' => 0,
    //     //     ];
    //     //     $total_damage += $item->total_amount;
    //     // }
    ...
    //         'fifoDetails', 'total_sales', 'total_purchases', 'openingStocks', 'start_date', 'end_date', 'products'
    //     ));
    // }
    ...
    //     // $sales = OrderDetail::whereIn('product_id', $product_id)
    //     //             ->where('delivery_status', 'delivered') 
    //     //             ->whereBetween('created_at', [$start_date, $end_date])
    //     //             ->get();
    //     $main_orders = OrderDetail::select('order_details.id', 'order_details.order_id', 'order_details.product_id', 'order_details.price', 'order_details.quantity as qty', 'order_details.delivery_status', 'orders.warehouse', 'order_details.created_at', 'order_details.updated_at')
                ...
                for ($year = $currentYear; $year >= $startYear; $year--) {
                    $products = Product::join('categories', 'products.category_id', '=', 'categories.id')
                        ...
                        ->select(
                            ...
                            'products.id as productId',
                            'categories.name as category_name',
                            'purchase_details.price as sale_price',
                            'purchase_details.qty as order_qty',
                            'purchase_details.created_at',
                            'purchases.id as poId',
                            'purchases.wearhouse_id',
    ...
    public function warehouse_stock_summery(Request $request)
    {
        ...
        $warehouses = Warehouse::when(!empty($warehouseIds), function ($query) use ($warehouseIds) {
            return $query->whereIn('id', $warehouseIds);
        })->get();

        $yearData = [];
        $previousYearClosingStocks = []; // Store previous year's closing stock for display
        ...
        foreach (range($startYear, $currentYear) as $year) {
            ...
            foreach ($warehouses as $warehouse) {
                ...
                // Opening stocks should only be from the start month (January) of the year
                $openingStockStart = $year . '-01-01';
                $openingStockEnd = $year . '-01-31';
                
                $opening_stocks = OpeningStock::where('wearhouse_id', $warehouseId)
                    ->whereIn('product_id', $product_ids)
                    ->whereBetween('created_at', [$openingStockStart, $openingStockEnd])
                    ->get()
                    ->groupBy('product_id');

                $purchases = PurchaseDetail::select('purchase_details.*', 'purchases.status', 'purchases.date')
                    ->leftJoin('purchases', 'purchases.id', '=', 'purchase_details.purchase_id')
                    ->where('purchases.status', 2)
                    ->whereIn('purchase_details.product_id', $product_ids)
                    ->where('purchase_details.wearhouse_id', $warehouseId)
                    ->whereBetween('purchases.date', [$yearStart, $yearEnd])
                    ->get()
                    ->groupBy('product_id');
                ...
                $main_refunds = RefundRequest::select('refund_requests.id', 'order_details.product_id', 'refund_requests.return_qty as qty', 'refund_requests.return_amount as amount', 'refund_requests.purchase_price as price')
                    ...
                    ->whereBetween('refund_requests.created_at', [$yearStart . ' 00:00:00', $yearEnd . ' 23:59:59'])
                    ->get();
                ...
                $main_orders = OrderDetail::select('order_details.id', 'order_details.product_id', 'order_details.price', 'order_details.quantity as qty', 'orders.warehouse', 'orders.delivered_date')
                    ->leftJoin('orders', 'orders.id', '=', 'order_details.order_id')
                    ->where('order_details.delivery_status', 'delivered')
                    ->whereIn('order_details.product_id', $product_ids)
                    ->where('orders.warehouse', $warehouseId)
                    ->whereBetween('orders.delivered_date', [$yearStart . ' 00:00:00', $yearEnd . ' 23:59:59'])
                    ->get();

                $child_orders = OrderDetail::select('order_details.id', 'products.parent_id as product_id', 'order_details.price', DB::raw('products.deduct_qty * order_details.quantity as qty'), 'orders.warehouse', 'orders.delivered_date')
                    ->leftJoin('products', 'products.id', '=', 'order_details.product_id')
                    ->leftJoin('orders', 'orders.id', '=', 'order_details.order_id')
                    ->where('order_details.delivery_status', 'delivered')
                    ->whereIn('products.parent_id', $product_ids)
                ...
                $damages = Damage::select('damages.id', 'damages.product_id', 'damages.wearhouse_id', 'damages.qty', 'damages.total_amount as amount', 'damages.status', 'damages.date')
                    ->whereIn('product_id', $product_ids)
                    ->where('status', 'Approved')
                    ->where('wearhouse_id', $warehouseId)
                    ->whereBetween('date', [$yearStart, $yearEnd])
                    ->get()
                    ->groupBy('product_id');
                ...
                // Collect sums across all products
                $openingStockAmount = $products->sum('opening_stock_amount');
                $purchaseAmount = $products->sum('purchase_amount');
                $transferInAmount = $products->sum('receive_amount');
                $salesAmount = $products->sum('sales_amount');
                $transferOutAmount = $products->sum('transfer_amount');
                $closingStockAmount = $products->sum('closing_stock_amount');

                // Check if we should display previous year's closing stock as opening stock
                $displayOpeningStock = $openingStockAmount;
                if ($year > $startYear && isset($previousYearClosingStocks[$warehouseId])) {
                    $displayOpeningStock = $previousYearClosingStocks[$warehouseId];
                }

                $profitLoss = $salesAmount - (($openingStockAmount + $purchaseAmount + $transferInAmount) - ($transferOutAmount + $closingStockAmount));

                $data = [
                    'opening_stock' => $displayOpeningStock, // Display previous year's closing as opening
                ...
                foreach ($data as $key => $val) {
                    $yearData[$year]['totals'][$key] += $val;
                }

                // Store closing stock for next year's opening stock display
                $previousYearClosingStocks[$warehouseId] = $closingStockAmount;
            }
        }
    ...
    public function monthly_warehouse_stock_summery(Request $request, $year)
    {
        ...
        // Determine the maximum month to process
        $currentYear = date('Y');
        ...
        foreach (range(1, 12) as $month) {
            ...
            // Skip processing for future months in the current year
            if ($month > $maxMonth) {
                foreach ($warehouses as $warehouse) {
                    $monthlyData[$month]['warehouses'][] = [
                        'name' => $warehouse->name,
                        'data' => [
                            'opening_stock' => 0,
                            'purchase' => 0,
            ...
            foreach ($warehouses as $warehouse) {
                ...
                $main_refunds = RefundRequest::select('refund_requests.id', 'order_details.product_id', 'refund_requests.return_qty as qty', 'refund_requests.return_amount as amount', 'refund_requests.purchase_price as price')
                    ...
                    ->where('refund_requests.refund_status', 5)
                    ->where('orders.warehouse', $warehouseId)
                    ->whereYear('refund_requests.created_at', $year)
                    ->whereMonth('refund_requests.created_at', $month)
                    ->get();
                ...
                foreach ($data as $key => $value) {
                    $monthlyData[$month]['totals'][$key] += $value;
                }
            }
        }
    ...
    public function single_employee_sales_performance(Request $request, $type = null)
    {
        ...
        return view('backend.reports.single_employee_sales_performance', compact('orders', 'start_date', 'end_date', 'months', 'currentYear', 'totals', 'user_id', 'selectedUserName', 'overallTotalSales'));
    }
    ...
    public function employee_sales_performance_compare(Request $request)
    {
        ...
        return view('backend.reports.employee_sales_performance_compare', compact('employeeData', 'start_date', 'end_date', 'currentYear', 'users', 'user_ids'));
    }
    ...
    public function daily_sale_profit_report(Request $request)
    {
        ...
        $warehouseFilter = !empty($wearhouse) ? (array)$wearhouse : null;
        ...
        foreach ($products as $product) {
            ...
            $stockQueue = [];
            $addStockToQueue = function ($units, $unitCost) use (&$stockQueue) {
                if ($units <= 0) {
                    return;
                }
                $stockQueue[] = [
            ...
            $deductStockFromQueue = function ($units, $fallbackCost) use (&$stockQueue) {
                $deductedCost = 0;
                $remaining = (float)$units;
                $fallback = (float)$fallbackCost;

                while ($remaining > 0) {
                    if (empty($stockQueue)) {
                        $deductedCost += $remaining * $fallback;
                        break;
                    }

                    $currentLayer = &$stockQueue[0];
                    if ($currentLayer['units'] <= $remaining) {
                        $deductedCost += $currentLayer['units'] * $currentLayer['cost'];
                        $remaining -= $currentLayer['units'];
                        array_shift($stockQueue);
                    } else {
                        $deductedCost += $remaining * $currentLayer['cost'];
                        $currentLayer['units'] -= $remaining;
                        $remaining = 0;
                    }
                }
            ...
            $buildTransactions = function (string $range) use (
                $productId,
                $warehouseFilter,
                $start_date,
                $end_date,
                ...
                $timestampFilter = function ($columnExpression, $query, $range, $start_date, $end_date) {
                    if ($range === 'before') {
                        $query->whereRaw("$columnExpression < ?", [$start_date]);
                    } else {
                        $query->whereRaw("$columnExpression BETWEEN ? AND ?", [$start_date, $end_date]);
                    }
                };

                // Opening Stocks
                $openingQuery = OpeningStock::where('product_id', $productId)
                    ->when($warehouseFilter, function ($query) use ($warehouseFilter) {
                        $query->whereIn('wearhouse_id', $warehouseFilter);
                    })
                    ->orderBy('created_at', 'asc');

                $timestampFilter('opening_stocks.created_at', $openingQuery, $range, $start_date, $end_date);

                foreach ($openingQuery->get() as $stock) {
                    $transactions->push([
                        'timestamp' => strtotime($stock->created_at),
                        'direction' => 'receipt',
                        'source' => 'opening',
                        'qty' => (float)$stock->qty,
                        'unit_cost' => (float)$stock->price
                    ]);
                }

                // Purchases
                $purchaseQuery = PurchaseDetail::where('product_id', $productId)
                    ->select(
                        'purchase_details.qty',
                        'purchase_details.price',
                        'purchase_details.wearhouse_id',
                        'purchases.date'
                    )
                    ->join('purchases', 'purchases.id', '=', 'purchase_details.purchase_id')
                    ->where('purchases.status', 2)
                    ->when($warehouseFilter, function ($query) use ($warehouseFilter) {
                        $query->whereIn('purchase_details.wearhouse_id', $warehouseFilter);
                    })
                    ->orderBy('purchases.date', 'asc');

                if ($range === 'before') {
                    $purchaseQuery->where('purchases.date', '<', $startDate);
                } else {
                    $purchaseQuery->whereBetween('purchases.date', [$startDate, $endDate]);
                }

                foreach ($purchaseQuery->get() as $purchase) {
                    $purchaseTimestamp = $purchase->date ?? $start_date;
                    $transactions->push([
                        'timestamp' => strtotime($purchaseTimestamp),
                        'direction' => 'receipt',
                        'source' => 'purchase',
                        'qty' => (float)$purchase->qty,
                        'unit_cost' => (float)$purchase->price
                    ]);
                }

                // Transfers In
                $transferInQuery = Transfer::where('product_id', $productId)
                    ->where('status', 'Approved')
                    ->when($warehouseFilter, function ($query) use ($warehouseFilter) {
                        $query->whereIn('to_wearhouse_id', $warehouseFilter);
                    })
                    ->orderBy('date', 'asc');

                $timestampFilter('transfers.date', $transferInQuery, $range, $start_date, $end_date);

                foreach ($transferInQuery->get() as $transferIn) {
                    $transactions->push([
                        'timestamp' => strtotime($transferIn->date),
                        'direction' => 'receipt',
                        'source' => 'transfer_in',
                        'qty' => (float)$transferIn->qty,
                        'unit_cost' => (float)$transferIn->unit_price
                    ]);
                }

                // Refunds (treated as receipt back to stock)
                $refundQuery = RefundRequest::select(
                    ...
                    ->when($warehouseFilter, function ($query) use ($warehouseFilter) {
                        $query->whereIn('orders.warehouse', $warehouseFilter);
                    })
                    ->orderBy('refund_requests.created_at', 'asc');

                $timestampFilter('refund_requests.created_at', $refundQuery, $range, $start_date, $end_date);

                foreach ($refundQuery->get() as $refund) {
                    $qty = (float)$refund->qty;
                    $unitCost = $qty > 0 ? ((float)$refund->amount / $qty) : 0;
                    $transactions->push([
                        'timestamp' => strtotime($refund->created_at),
                        'direction' => 'receipt',
                        'source' => 'refund',
                        'qty' => $qty,
                        'unit_cost' => $unitCost
                ...
                $timestampFilter('orders.delivered_date', $salesQuery, $range, $start_date, $end_date);

                foreach ($salesQuery->get() as $sale) {
                    $transactions->push([
                        'timestamp' => strtotime($sale->delivered_date ?? $start_date),
                        'direction' => 'issue',
                        'source' => 'sale',
                ...
                // Transfers Out
                $transferOutQuery = Transfer::where('product_id', $productId)
                    ->where('status', 'Approved')
                    ->when($warehouseFilter, function ($query) use ($warehouseFilter) {
                        $query->whereIn('from_wearhouse_id', $warehouseFilter);
                    })
                    ->orderBy('date', 'asc');

                $timestampFilter('transfers.date', $transferOutQuery, $range, $start_date, $end_date);

                foreach ($transferOutQuery->get() as $transferOut) {
                    $transactions->push([
                        'timestamp' => strtotime($transferOut->date),
                        'direction' => 'issue',
                        'source' => 'transfer_out',
                        'qty' => (float)$transferOut->qty
                    ]);
                }

                // Damages
                $damageQuery = Damage::where('product_id', $productId)
                    ->where('status', 'Approved')
                    ->when($warehouseFilter, function ($query) use ($warehouseFilter) {
                        $query->whereIn('wearhouse_id', $warehouseFilter);
                    })
                ...
                $timestampFilter('damages.date', $damageQuery, $range, $start_date, $end_date);

                foreach ($damageQuery->get() as $damage) {
                    $transactions->push([
                        'timestamp' => strtotime($damage->date),
                        'direction' => 'issue',
                        'source' => 'damage',
                        'qty' => (float)$damage->qty
                    ]);
                }

                return $transactions->sortBy('timestamp')->values();
            };

            // Seed opening balance with transactions before the date range
            $previousTransactions = $buildTransactions('before');
            foreach ($previousTransactions as $transaction) {
                if ($transaction['direction'] === 'receipt') {
                    $addStockToQueue($transaction['qty'], $transaction['unit_cost']);
                } else {
                    $deductStockFromQueue($transaction['qty'], $product->purchase_price ?? 0);
                }
            }

            $openingStockQty = array_sum(array_column($stockQueue, 'units'));
            $openingStockAmount = 0;
            foreach ($stockQueue as $layer) {
                $openingStockAmount += $layer['units'] * $layer['cost'];
            }

            // Track metrics within the selected date range
            $purchaseQty = 0;
            $purchaseAmount = 0;
            $transferInQty = 0;
            $transferInAmount = 0;
            ...
            $currentTransactions = $buildTransactions('current');
            foreach ($currentTransactions as $transaction) {
                if ($transaction['direction'] === 'receipt') {
                    ...
                    switch ($transaction['source']) {
                        case 'purchase':
                            $purchaseQty += $transaction['qty'];
                            $purchaseAmount += $transaction['qty'] * $transaction['unit_cost'];
                            break;
                        case 'transfer_in':
                            $transferInQty += $transaction['qty'];
                            $transferInAmount += $transaction['qty'] * $transaction['unit_cost'];
                            break;
                        case 'refund':
                            // Treat refunds as receipts; no separate accounting required for purchases.
                            break;
                        case 'opening':
                        default:
                            break;
                    ...
                    $cost = $deductStockFromQueue($transaction['qty'], $product->purchase_price ?? 0);

                    switch ($transaction['source']) {
                        case 'sale':
                            $salesQty += $transaction['qty'];
                            $salesCost += $cost;
                            break;
                        case 'transfer_out':
                            $transferOutQty += $transaction['qty'];
                            $transferOutAmount += $cost;
                            break;
                        case 'damage':
                            $damageQty += $transaction['qty'];
                            $damageAmount += $cost;
                            break;
                        default:
                            break;
            ...
            $closingStockQty = array_sum(array_column($stockQueue, 'units'));
            $closingStockAmount = 0;
            foreach ($stockQueue as $layer) {
                $closingStockAmount += $layer['units'] * $layer['cost'];
            }

            $purchaseUnitPrice = $salesQty > 0 ? $salesCost / $salesQty : 0;
            ...
            $product->damage_qty = $damageQty;
            $product->damage_amount = $damageAmount;
            $product->quantity = $salesQty;
            $product->closing_stock_qty = $closingStockQty;
            $product->closing_stock_amount = $closingStockAmount;
            $product->purchase_unit_price = $purchaseUnitPrice;
            $product->purchase_total = $salesCost;
            $product->profit_loss = $product->total_order_price - $salesCost;
        }
    ...
    public function salesReport(Request $request)
    {
        ...
        $orders = Order::leftJoin('order_details', 'orders.id', '=', 'order_details.order_id')
            ...
            ->orderBy('orders.date', 'ASC');  // Changed from 'delivered_date' to 'date'
    ...
    public function PlatformSalesReport(Request $request, $type = null)
    {
        ...
        if (!empty($wearhouse)) {
            ...
            if (Auth::user()->id == 9 || Auth::user()->id == 135 || Auth::user()->id == 137 || Auth::user()->id == 138 || Auth::user()->id == 161431) {
                $orders = $orders->whereBetween('orders.created_at', [$start_date, $end_date]);
            } else {
                $orders = $orders->whereBetween('orders.created_at', [$start_date, $end_date])
                    ->where('orders.warehouse', $warehousearray);
    ...
    public function product_wise_purchase_report(Request $request, $type = null)
    {
        ...
        if (!empty($request->product_id)) {
            ...
            $pro_sort_by = ['all']; // To make "All" appear selected on first load
        }
        ...
        if (!empty($request->start_date) && !empty($request->end_date)) {
            $start_date = date('Y-m-d', strtotime($request->start_date));
            $end_date = date('Y-m-d', strtotime($request->end_date));
            $product_wise_purchase_list = $product_wise_purchase_list->whereBetween('purchases.date', [$start_date, $end_date]);
        } else {
            $product_wise_purchase_list = $product_wise_purchase_list->whereBetween('purchases.date', [$start_date, $end_date]);
        }
    ...
    public function employee_performance(Request $request)
    {
        ...
        if ($request->role == 9) {
            ...
            // Get all sales executives with role_id = 9
            ...
            if (!empty($user_id)) {
                $staffQuery->where('staff.user_id', $user_id);
            }

            // Left join with targets to include executives without targets
            $targets = $staffQuery->leftJoin('targets', function($join) use ($month_start, $month_end) {
                $join->on('staff.user_id', '=', 'targets.user_id')
                     ->whereBetween('targets.month', [$month_start, $month_end]);
            })
            ->selectRaw('staff.user_id, 
                users.name,
                roles.name as role_name,
                ...
                MAX(targets.month) as month')
            ->groupBy('staff.user_id', 'users.name', 'roles.name')
            ...
            foreach ($targets as $key => $com) {
                ...
                // Set default values for targets if null
                $targets[$key]->total_target = $com->total_target ?? 0;
                $targets[$key]->total_recovery_target = $com->total_recovery_target ?? 0;
                $targets[$key]->total_target_customer = $com->total_target_customer ?? 0;
                $targets[$key]->total_customer_achievement = $com->total_customer_achievement ?? 0;
                ...
                $targets[$key]->total_sales = $orders ?? 0;


                if (isset($com->total_target) && $com->total_target !== 0 && $com->total_target !== null) {
                    $sales_achievement = round($targets[$key]->total_sales * 100 / ($targets[$key]->total_target ?: 1), 2);
                } else {
                    $sales_achievement = 0;
                }
                ...
                $targets[$key]->customer_count = $customerCount ?? 0;

                if (isset($com->total_target_customer) && $com->total_target_customer !== 0 && $com->total_target_customer !== null) {
                    $customer_achivement = round(($customerCount * 100) / ($targets[$key]->total_target_customer ?: 1), 2);
                } else {
                ...
                $targets[$key]->monthlyTotaldue = ($monthlyDebit ?? 0) - ($monthlyTotaldue ?? 0);
            }
        ...
        if ($request->role == 13) {
            ...
            foreach ($delivery_staffs_main as $key => $com) {
                ...
                $total_supplier_credit = DB::select($total_supplier_cr);
