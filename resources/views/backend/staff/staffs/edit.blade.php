            ...
            <form action="{{ route('staffs.update', $staff->id) }}" method="POST">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <select name="warehouse_id[]" required class="form-control aiz-selectpicker" multiple>
                                @foreach($warehouses as $warehouse)
                                    <option @if($warehousearray != null && in_array($warehouse->id, $warehousearray)) selected @endif value="{{$warehouse->id}}">{{$warehouse->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
