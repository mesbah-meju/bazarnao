...
@section('content')
    <div class="aiz-titlebar text-left mt-2 mb-3">
        <div class=" align-items-center">
            <h1 class="h3">{{ translate('Product Wise Sales History Report') }}</h1>
        </div>
    </div>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="printArea">
                        ...
                        <h3 style="text-align:center;">{{ translate('Product Wise Sales History Report') }} <span
                                style="font-size: 16px">({{ $start_date }} to {{ $end_date }})</span></h3>
                                    ...
                                    <tr>
                                        ...
                                        {{-- <td>
                                            @if ($product->delivered_date)
                                                ...
                                                @endphp
                                                <a href="{{ route('product_wise_daily_sales_history_report.index', ['productId' => $product->productId, 'start_date' => $start_date, 'end_date' => $end_date]) }}"
                                                target="_blank">
                                        ...
                                        <td>
                                            @if ($product->delivered_date)
                                                @php
                                                    $delivered_date = date('Y-m-d', strtotime($product->delivered_date));
                                                @endphp
                                                <a href="{{ route('product_specific_day_sales_history_report.index', ['productId' => $product->productId, 
                                                'warehouse' => $product->warehouse,
                                                'start_date' => $delivered_date, 
                                                'end_date' => $delivered_date, 
                                                'delivered_date' => $delivered_date]) }}" target="_blank">
                                                    {{ $product->getTranslation('product_name') }}
                                                </a>
                                            @else
                                                {{ $product->getTranslation('product_name') }}
                                        ...
                                        <td>
                                            {{ $product->delivered_date ? date('F Y', strtotime($product->delivered_date)) : 'undefined' }}
                                            {{-- @if ($product->delivered_date)
                                                @php
                                                    $delivered_month = date('F Y', strtotime($product->delivered_date));
                                                    $start_date = date('Y-m-01', strtotime($product->delivered_date));
                                                    $end_date = date('Y-m-t', strtotime($product->delivered_date));
                                                @endphp
                                                <a href="{{ route('product_wise_daily_sales_history_report.index', ['productId' => $product->productId, 
                                                'warehouse' => $product->warehouse,
