    ...
    <div class="col-lg-12 mx-auto">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0 h6">{{translate('Edit Opening Balance')}}</h5>
            </div>
            ...
            <form class="form-horizontal" action="{{ route('opening-balances.update', $opening_balance->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-4">
                            <select name="warehouse_id" id="warehouse_id" class="form-control aiz-selectpicker" data-live-search="true" required>
                                ...
                                <option  value="{{ $warehouse->id }}" @selected($opening_balance->warehouse_id==$warehouse->id)>{{ $warehouse->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-4">
                            <select name="fyear" id="fyear" class="form-control aiz-selectpicker">
                                ...
                                <option value="{{ $oldyear->id }}" @selected($opening_balance->fyear == $oldyear->id)>{{ $oldyear->year_name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-4">
                            <input type="text" name="dtpDate" id="dtpDate" class="form-control datepicker" value="{{ date('m/d/Y', strtotime($opening_balance->open_date)) }}">
                        </div>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover" id="debtAccVoucher">
                            ...
                            <tbody id="debitvoucher">
                                <tr>
                                    <td>
                                        <select name="cmbCode[]" required id="cmbCode_1" class="form-control aiz-selectpicker" onchange="load_subtypeOpen(this.value,1)">
                                            <option value="">Select Option</option>
                                            @foreach ($acc as $acc1)
                                            <option value="{{ $acc1->head_code}}" @selected($opening_balance->coa_id == $acc1->head_code)>{{ $acc1->head_name }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    @if ($opening_balance->sub_type != 1)
                                    <td>
                                        <select name="subtype[]" id="subtype_1" class="form-control aiz-selectpicker" required>
                                            @foreach ($opening_balance->subtypes as $subtype)
                                                <option value="{{ $subtype->id }}" @selected($opening_balance->sub_code == $subtype->id)>{{ $subtype->name }}</option>
                                            @endforeach
                                        </select>
                                        <input type="hidden" name="isSubtype[]" id="isSubtype_1" value="{{ $opening_balance->sub_type }}" />
                                    </td>
                                    @else
                                    <td>
                                        <select name="subtype[]" id="subtype_1" class="form-control aiz-selectpicker" required>
                                    ...
                                    @endif
                                    <td>
                                        <input type="number" name="txtDebit[]" value="{{ ($opening_balance->debit != 0) ? $opening_balance->debit : '' }}" class="form-control total_dprice text-right" id="txtDebit_1" onkeyup="calculationDebtOpen(1)">
                                    </td>
                                    <td>
                                        <input type="number" name="txtCredit[]" value="{{ ($opening_balance->credit != 0) ? $opening_balance->credit : '' }}" class="form-control total_cprice text-right" id="txtCredit_1" onkeyup="calculationCreditOpen(1)">
                                        <input type="hidden" name="isSubtype[]" id="isSubtype_1" value="{{ $opening_balance->sub_type }}" />
                                    </td>
                                    <td>
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <td class="text-right">
                                        <input type="text" id="grandTotald" class="form-control text-right " name="grand_totald" value="{{ ($opening_balance->debit != 0) ? $opening_balance->debit : '' }}" readonly="readonly" />
                                    </td>
                                    <td class="text-right">
                                        <input type="text" id="grandTotalc" class="form-control text-right " name="grand_totalc" value="{{ ($opening_balance->credit != 0) ? $opening_balance->credit : '' }}" readonly="readonly" />
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
                    ...
                    <div class="form-group form-group-margin row">
                        <div class="col-sm-12 text-right">
                            <input type="submit" id="add_receive" class="btn btn-success btn-large" name="save" value="{{ translate('Update') }}" tabindex="9" />
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        setTimeout(function() {
            var existingHeadCode = $('#cmbCode_1').val();
            if(existingHeadCode) {
                setReadonlyByHead(existingHeadCode, 1);
            }
        }, 500);
    });
    ...
    function setReadonlyByHead(headCode, sl) {
        $.ajax({
            ...
            success: function(resp) {
                ...
                if (!details) {
                    debitEl.prop('readonly', false);
                    creditEl.prop('readonly', false);
                    return;
                }
                ...
                if (headType === 'A' || headType === 'E') {
                    debitEl.prop('readonly', false);
                    creditEl.prop('readonly', true);
                    if(creditEl.val() === '' || creditEl.val() === null) {
                        creditEl.val('0');
                    }
                } else if (headType === 'L' || headType === 'I') {
                    creditEl.prop('readonly', false);
                    debitEl.prop('readonly', true);
                    if(debitEl.val() === '' || debitEl.val() === null) {
                        debitEl.val('0');
                    }
                } else {
                    debitEl.prop('readonly', false);
