    ...
    <div class="col-lg-12 mx-auto">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0 h6">{{translate('Add Credit Voucher')}}</h5>
            </div>
            ...
            <form class="form-horizontal" action="{{ route('credit-vouchers.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="card-body">
                    <div class="form-group row">
                        <label for="vo_no" class="col-sm-2 col-form-label">{{ translate('Voucher Type') }} <span class="text-danger">*</span></label>
                        <div class="col-sm-4">
                            <input type="text" name="txtVNo" id="txtVNo" value="Credit" class="form-control" readonly required/>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        <label for="date" class="col-sm-2 col-form-label">{{ translate('Date') }} <span class="text-danger">*</span></label>
                        <div class="col-sm-4">
                            ...
                            <tbody id="creditvoucher">
                                <tr>
                                    <td class="expenseincometd">
                                        <select name="cmbCode[]" id="cmbCode_1" required class="form-control aiz-selectpicker" data-live-search="true" onchange="load_subtypeCreditv(this.value,1)">
                                    ...
                                    <td>
                                        <select name="subtype[]" id="subtype_1" required class="form-control aiz-selectpicker" data-live-search="true">
                                            <option value="">Select Option</option>
                                        </select>
                                    </td>
                                    <td class="reltype">
                                    ...
                                    <td>
                                        <input type="number" name="txtAmount[]" required step=".01" value="" class="form-control total_price text-right" id="txtAmount_1" onkeyup="calculationCreditv(1)">
                                    </td>
                                    <td>
...
<script type="text/javascript">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const creditVoucherForm = document.querySelector('form[action="{{ route('credit-vouchers.store') }}"]');

        if (creditVoucherForm) {
            creditVoucherForm.addEventListener('submit', function (e) {
                e.preventDefault(); // Stop form from submitting immediately

                Swal.fire({
                    title: 'Are you sure?',
                    text: "Do you want to save this credit voucher?",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        creditVoucherForm.submit(); // Submit the form if confirmed
                    }
                });
            });
        }
