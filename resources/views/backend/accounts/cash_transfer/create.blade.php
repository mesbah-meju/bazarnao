    ...
    $(document).ready(function () {
        ...
        $('#txtVNo').on('change', function () {
            ...
            if (voucherType === 'Cash to Cash') {
                ...
                @foreach ($crcc as $cracc)
                    $('#cmbDebit').append(new Option('{{ $cracc->head_name }}', '{{ $cracc->head_code }}'));
                @endforeach
            } else if (voucherType === 'Cash to Bank') {
                // Use $crcc2 for Cash to Bank
                @foreach ($crcc2 as $cracc2)
                    $('#cmbDebit').append(new Option('{{ $cracc2->head_name }}', '{{ $cracc2->head_code }}'));
                @endforeach
            }
        ...
        // Initialize Account Head for To Warehouse on page load
