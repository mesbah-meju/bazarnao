          ...
          <a href="javascript:" id="add_row" class="btn btn-sm btn-primary"><i class="las la-plus"></i>Add Row</a>
                    ...
                    <a href="javascript:" onclick="removeItemRow({{$key+1}})" class="btn btn-sm btn-danger"><i class="las la-minus"></i></a>
                        ...
                        @if($product->is_group_product)
                            ...
                            $disAmount = App\Models\Group_product->where('group_product_id',$product->id)->sum('discount_amount');
                            $acPrice = App\Models\Group_product->where('group_product_id',$product->id)->sum('price');
                        @else
    			...
    			<table class="table">
        			...
        			<tr>
                        ...
                        <td class="text-muted h5">
                            <input class="form-control" type="text" name="grand_total" id="grand_total" value="{{ $order->grand_total - $order->coupon_discount }}">
        				</td>
        			</tr>
        			</tbody>
    			</table>
...
@section('script')
    ...
    if($product->is_group_product) {
        // Sum the price for the group product
        $acPrice = App\Models\Group_product::where('group_product_id', $product->id)->sum('price');
    } else {
        if($product->discount > 0) {
            if($product->discount_type == 'amount') {
