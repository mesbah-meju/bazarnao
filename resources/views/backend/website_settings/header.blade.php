			...
			<div class="card-body">
				<form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
					...
					<div class="form-group row">
	                    <label class="col-md-3 col-from-label">{{ translate('Header Logo English') }}</label>
						<div class="col-md-8">
					...
					<div class="form-group row">
	                    <label class="col-md-3 col-from-label">{{ translate('Header Logo Bangla') }}</label>
						<div class="col-md-8">
		                    <div class=" input-group " data-toggle="aizuploader" data-type="image">
		                        <div class="input-group-prepend">
		                            <div class="input-group-text bg-soft-secondary font-weight-medium">{{ translate('Browse') }}</div>
		                        ...
		                        <div class="form-control file-amount">{{ translate('Choose File') }}</div>
								<input type="hidden" name="types[]" value="header_logo_bangla">
		                        <input type="hidden" name="header_logo_bangla" class="selected-files" value="{{ get_setting('header_logo_bangla') }}">
		                    </div>
		                    <div class="file-preview"></div>
						</div>
	                </div>
					...
					<div class="border-top pt-3">
						...
						<div class="header-nav-menu">
							...
							@if (get_setting('header_menu_labels') != null)
								@foreach (json_decode(App\Models\BusinessSetting::where('type', 'header_menu_labels')->first()->value, true) as $key => $value)
