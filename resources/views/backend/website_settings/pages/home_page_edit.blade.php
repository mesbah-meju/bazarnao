	...
	<div class="col-xl-10 mx-auto">
		...
		<div class="card">
			<div class="card-header">
				<h6 class="mb-0">{{ translate('Stories') }}</h6>
			</div>
			<div class="card-body">
				<form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
					@csrf
					<div class="form-group">
						<label>{{ translate('Upload Story Files (Image/Video), Links & Names') }}</label>
						<div class="story-upload-target">
							<input type="hidden" name="types[]" value="story_files">
							<input type="hidden" name="types[]" value="story_links">
							<input type="hidden" name="types[]" value="story_names">
							@php
							$files = get_setting('story_files') ? json_decode(get_setting('story_files'), true) : [];
							$links = get_setting('story_links') ? json_decode(get_setting('story_links'), true) : [];
							$names = get_setting('story_names') ? json_decode(get_setting('story_names'), true) : [];
							@endphp
							...
							<div class="row gutters-5">
								<div class="col-md-4">
									<div class="form-group">
										<div class="input-group" data-toggle="aizuploader" data-type="all">
											...
											<input type="hidden" name="types[]" value="story_files">
											<input type="hidden" name="story_files[]" class="selected-files" value="{{ $value }}">
										</div>
								...
								<div class="col-md-4">
									<div class="form-group">
										<input type="hidden" name="types[]" value="story_links">
										<input type="text" class="form-control" placeholder="http://" name="story_links[]" value="{{ $links[$key] ?? '' }}">
									</div>
								</div>
								<div class="col-md-3">
									<div class="form-group">
										<input type="hidden" name="types[]" value="story_names">
										<input type="text" class="form-control" placeholder="{{ translate('Story Name') }}" name="story_names[]" value="{{ $names[$key] ?? '' }}">
									</div>
								</div>
                    ...
                    <div class="row gutters-5">
                        <div class="col-md-4">
                            <div class="form-group">
                                <div class="input-group" data-toggle="aizuploader" data-type="all">
                                    ...
                                    <div class="form-control file-amount">{{ translate("Choose File") }}</div>
                                    <input type="hidden" name="types[]" value="story_files">
                                    <input type="hidden" name="story_files[]" class="selected-files">
                                </div>
                                <div class="file-preview box sm"></div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <input type="hidden" name="types[]" value="story_links">
                                <input type="text" class="form-control" placeholder="http://" name="story_links[]">
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="form-group">
                                <input type="hidden" name="types[]" value="story_names">
                                <input type="text" class="form-control" placeholder="{{ translate("Story Name") }}" name="story_names[]">
                            </div>
                        </div>
                        <div class="col-md-auto">
                            <div class="form-group">
							...
							data-target=".story-upload-target">
							{{ translate('Add New') }}
						</button>
					</div>
				...
				<form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
					...
					<div class="form-group row">
						...
						<div class="col-md-10">
							...
							<select name="top10_categories[]" class="form-control aiz-selectpicker" multiple data-live-search="true" data-selected={{ get_setting('top10_categories') }} required>
								@foreach (\App\Models\Category::where('parent_id', 0)->with('childrenCategories')->get() as $category)
								<option value="{{ $category->id }}">{{ $category->getTranslation('name') }}</option>
								@foreach ($category->childrenCategories as $childCategory)
								@include('categories.child_category', ['child_category' => $childCategory])
