        ...
        <table class="table aiz-table mb-0">
            <thead>
                <tr>
                    ...
                    <th>{{translate('Category Wise Product')}}</th>
            ...
            <tbody>
                @foreach($categories as $key => $category)
                    <tr>
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input type="checkbox" onchange="update_featured_category_wise(this)" value="{{ $category->id }}" <?php if($category->category_wise == 1) echo "checked";?>>
                                <span></span>
                            </label>
                        </td>
...
@section('script')
    <script type="text/javascript">
        ...
        function update_featured_category_wise(el){
            if(el.checked){
                var status = 1;
            }
            else{
            ...
            $.post('{{ route('categories.featured_category_wise') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Category Wise Product updated successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
