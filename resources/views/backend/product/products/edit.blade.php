	...
	<div class="">
		<form class="form form-horizontal mar-top" action="{{route('products.update', $product->id)}}" method="POST" enctype="multipart/form-data" id="choice_form">
			...
			<div class="card">
				...
				<div class="card-body">
					...
					<div class="form-group row" id="parent">
	                    <label class="col-lg-3 col-from-label">{{translate('Parent')}}</label>
	                    <div class="col-lg-8">
	                        <select class="form-control aiz-selectpicker" name="parent_id" id="parent_id" data-live-search="true">
								...
								@foreach (\App\Models\Product::all() as $prod)
									<option value="{{ $prod->id }}" @if($product->parent_id == $prod->id)selected @endif>{{ $prod->getTranslation('name') }}</option>
								@endforeach
	                        </select>
	                    </div>
	                </div>
					...
					<div class="form-group row">
	                    <label class="col-lg-3 col-from-label">{{translate('Deduct QTY')}} <i class="las la-language text-danger" title="{{translate('Translatable')}}"></i> </label>
	                    <div class="col-lg-8">
	                        <input type="text" class="form-control" name="deduct_qty" value="@if($product->deduct_qty <= 1){{1}}@else{{$product->deduct_qty}}@endif" min="1">
	                    </div>
	                </div>
	                ...
	                <div class="form-group row">
						<label class="col-lg-3 col-from-label">{{translate('Minimum Qty')}}<span class="text-danger">*</span></label>
						<div class="col-lg-8">
					...
					<div class="form-group row">
						<label class="col-md-3 col-from-label">{{translate('Maximum Web Order Qty')}} <span class="text-danger">*</span></label>
						<div class="col-md-8">
							<input type="number" lang="en" class="form-control" name="max_qty" value="{{$product->max_qty}}" min="1" required>
						</div>
					</div>
					<div class="form-group row">
						<label class="col-md-3 col-form-label">{{ translate('Maximum App Order Qty') }}<span class="text-danger">*</span></label>
						<div class="col-md-8">
							<input type="number" lang="en" class="form-control" name="app_max_qty" value="{{$product->app_max_qty}}" min="1" required>
						</div>
                    </div>
	                <div class="form-group row">
					...
					@endif
					<div class="form-group row">
						<label class="col-lg-3 col-from-label">{{translate('Out of Stock')}}</label>
						<div class="col-lg-8">
							<label class="aiz-switch aiz-switch-success mb-0" style="margin-top:5px;">
								<input type="checkbox" name="outofstock" @if ($product->outofstock == 1) checked @endif>
	                            <span class="slider round"></span></label>
							</label>
						</div>
					</div>
			...
			<div class="card">
				...
				<div class="card-body">
					...
					<div class="form-group row">
	                    ...
	                    <div class="col-lg-6">
	                        <input type="number" lang="en" min="0" step="0.01" placeholder="{{translate('Unit price')}}" name="unit_price" id="unit_price" class="form-control" value="{{$product->unit_price}}" required>
	                    </div>
	                </div>
					...
					<!-- Web Discount -->
					<div class="form-group row">
						<label class="col-md-3 col-from-label">{{translate('Web Discount')}} <span class="text-danger">*</span></label>
						<div class="col-md-3">
							<input type="number" lang="en" min="0" step="0.01" placeholder="{{translate('Web Discount')}}" name="discount" id="discount" class="form-control" value="{{ $product->discount }}" required>
						</div>
						<div class="col-md-3">
						...
						<div class="col-md-3">
							<input type="number" lang="en" min="0" step="0.01" placeholder="{{translate('Price after Discount')}}" name="web_price_after_discount" id="web_price_after_discount" class="form-control" value="{{ $product->web_price_after_discount }}" readonly>
						</div>
					</div>

					<!-- App Discount -->
					<div class="form-group row">
						<label class="col-md-3 col-from-label">{{translate('App Discount')}} <span class="text-danger">*</span></label>
						<div class="col-md-3">
							<input type="number" lang="en" min="0" step="0.01" placeholder="{{translate('App Discount')}}" name="app_discount" id="app_discount" class="form-control" value="{{ $product->app_discount }}" required>
						</div>
						<div class="col-md-3">
							<select class="form-control aiz-selectpicker" name="app_discount_type" id="app_discount_type" required>
								<option value="amount" @if($product->app_discount_type == 'amount') selected @endif>{{translate('Flat')}}</option>
								<option value="percent" @if($product->app_discount_type == 'percent') selected @endif>{{translate('Percent')}}</option>
							</select>
						</div>
						<div class="col-md-3">
							<input type="number" lang="en" min="0" step="0.01" placeholder="{{translate('Price after Discount')}}" name="app_price_after_discount" id="app_price_after_discount" class="form-control" value="{{ $product->app_price_after_discount }}" readonly>
						</div>
					</div>
					...
					<div class="form-group row">
						<label class="col-md-3 col-from-label">{{ $wearhouse->name }} Warehouse Discount <span class="text-danger">*</span></label>
						<div class="col-md-3">
							<input type="number" lang="en" min="0" step="0.01" placeholder="{{ $wearhouse->name }} Warehouse Discount" name="pos_discount_{{ $wearhouse->id }}" id="pos_discount_{{ $wearhouse->id }}" class="form-control" value="{{ $product->{'warehouse'.$wearhouse->id.'_discount'} }}" required>
						</div>
						<div class="col-md-3">
							<select class="form-control aiz-selectpicker" name="discount_type_{{ $wearhouse->id }}" id="discount_type_{{ $wearhouse->id }}" required>
								<option value="amount" @if($product->{'warehouse'.$wearhouse->id.'_discount_type'} == 'amount') selected @endif>{{translate('Flat')}}</option>
								<option value="percent" @if($product->{'warehouse'.$wearhouse->id.'_discount_type'} == 'percent') selected @endif>{{translate('Percent')}}</option>
							</select>
						</div>
						<div class="col-md-3">
							<input type="number" lang="en" min="0" step="0.01" placeholder="{{translate('Price after Discount')}}" name="pos_price_after_discount_{{ $wearhouse->id }}" id="pos_price_after_discount_{{ $wearhouse->id }}" class="form-control" value="{{ $product->{'warehouse'.$wearhouse->id.'_price_after_discount'} }}" readonly>
						</div>
					</div>
					@endforeach
			...
			<div class="card">
				...
				<div class="card-body">
				<div class="form-group row">
						<label class="col-lg-3 col-from-label">{{translate('Shelf Name')}}</label>
						<div class="col-lg-6">
							<input type="text" value="{{ $product->self_name }}"  placeholder="{{translate('Shelf Name')}}" name="self_name" class="form-control" >
						</div>
					</div>
				<div class="form-group row">
						<label class="col-lg-3 col-from-label">{{translate('Shelf No')}}</label>
						<div class="col-lg-6">
							<input type="text" value="{{ $product->self_no }}"  placeholder="{{translate('Shelf Name')}}" name="self_no" class="form-control" >
						</div>
					</div>
					<div class="form-group row">
			...
			<div class="card">
				...
				<div class="card-body">
					<div class="form-group row">
						...
						<div class="col-md-8">
							<textarea class="aiz-text-editor" name="notice">{{ $product->notice }}</textarea>
						</div>
					</div>
				</div>
			</div>
...
<script type="text/javascript">
	...
	document.addEventListener("DOMContentLoaded", function() {
		function calculatePriceAfterDiscount(unitPrice, discountValue, discountType) {
			let priceAfterDiscount = unitPrice;
			if (discountType === "amount") {
				priceAfterDiscount -= discountValue;
			} else if (discountType === "percent") {
				priceAfterDiscount -= (unitPrice * (discountValue / 100));
			}
			return priceAfterDiscount.toFixed(2); 
		}

		function updatePriceAfterDiscount(discountTypeId, discountInputId, priceAfterDiscountId) {
			const unitPrice = parseFloat(document.getElementById('unit_price').value) || 0;
			const discountValue = parseFloat(document.getElementById(discountInputId).value) || 0;
			const discountType = document.getElementById(discountTypeId).value;
			const priceAfterDiscount = calculatePriceAfterDiscount(unitPrice, discountValue, discountType);
			document.getElementById(priceAfterDiscountId).value = priceAfterDiscount;
		}

		// Update on input and change events
		document.getElementById('discount').addEventListener('input', function() {
			updatePriceAfterDiscount('discount_type', 'discount', 'web_price_after_discount');
		});
		document.getElementById('discount_type').addEventListener('change', function() {
			updatePriceAfterDiscount('discount_type', 'discount', 'web_price_after_discount');
		});

		document.getElementById('app_discount').addEventListener('input', function() {
			updatePriceAfterDiscount('app_discount_type', 'app_discount', 'app_price_after_discount');
		});
		document.getElementById('app_discount_type').addEventListener('change', function() {
			updatePriceAfterDiscount('app_discount_type', 'app_discount', 'app_price_after_discount');
		});

		<?php foreach ($wearhouses as $i => $wearhouse) { ?>
			document.getElementById('pos_discount_<?php echo $wearhouse->id; ?>').addEventListener('input', function() {
				updatePriceAfterDiscount('discount_type_<?php echo $wearhouse->id; ?>', 'pos_discount_<?php echo $wearhouse->id; ?>', 'pos_price_after_discount_<?php echo $wearhouse->id; ?>');
			});
			document.getElementById('discount_type_<?php echo $wearhouse->id; ?>').addEventListener('change', function() {
				updatePriceAfterDiscount('discount_type_<?php echo $wearhouse->id; ?>', 'pos_discount_<?php echo $wearhouse->id; ?>', 'pos_price_after_discount_<?php echo $wearhouse->id; ?>');
			});
		<?php } ?>
