@php
    $offer_det = json_decode($offer->details);
@endphp
...
<div class="form-group row">
   ...
   <div class="col-lg-9">
       <input type="text" value="{{$offer->title}}" id="title" name="title" class="form-control" required>
   </div>
</div>
<div class="form-group row">
...
<div class="product-choose-list">
    <div class="product-choose">
        <div class="form-group row">
            <label class="col-lg-3 control-label" for="name">{{translate('Excluded Product')}}</label>
            <div class="col-lg-9">
                <select name="product_ids[]" class="form-control product_id aiz-selectpicker" data-live-search="true" data-selected-text-format="count" required multiple>
                    ...
                    @foreach(filter_products(\App\Models\Models\Product::query())->get() as $key => $product)
                        <option value="{{$product->id}}" 
                        @if(!empty($offer_det->product_id))
                            @if(in_array($product->id,$offer_det->product_id))
                                                {{'selected'}}    
                        @endif    
                        @endif    
...
<div class="form-group row">
  ...
  <div class="col-lg-9">
     <input type="number" lang="en" min="0" step="0.01" name="min_buy" class="form-control" value="{{ $offer_det->min_buy }}" required>
  </div>
</div>
<div class="form-group row">
   <label class="col-lg-3 col-from-label">{{translate('Discount')}}</label>
   <div class="col-lg-7">
       <input type="number" lang="en" min="0" step="0.01" placeholder="{{translate('Discount')}}" name="discount" class="form-control" value="{{ $offer->discount }}" required>
   </div>
   <div class="col-lg-2">
...
<div class="form-group row">
  ...
  <div class="col-lg-9">
     <input type="number" lang="en" min="0" step="0.01" placeholder="{{translate('Maximum Discount Amount')}}" name="max_discount" class="form-control" value="{{ $offer_det->max_discount }}" required>
  </div>
</div>
