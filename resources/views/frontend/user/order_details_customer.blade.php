...
<div class="modal-body gry-bg px-3 pt-3">
    <div class="py-4">
        <div class="row gutters-5 text-center aiz-steps">
            ...
            <div class="col @if($status == 'confirmed') active @elseif($status == 'on_delivery' || $status == 'delivered' || $status == 'received') done @endif">
                <div class="icon">
                    <i class="las la-newspaper"></i>
                </div>
              <div class="title fs-12">{{ translate('Confirmed')}}</div>
            </div>
            <div class="col @if($status == 'on_delivery') active @elseif($status == 'delivered' || $status == 'received') done @endif">
                <div class="icon">
                    <i class="las la-truck"></i>
                </div>
                <div class="title fs-12">{{ translate('On delivery')}}</div>
            </div>
            <div class="col @if($status == 'delivered' || $status == 'received') done @endif">
                <div class="icon">
                    <i class="las la-clipboard-check"></i>
                </div>
                <div class="title fs-12">{{ translate('Delivered')}}</div>
            </div>
            <div class="col @if($status == 'received') done @endif">
                <div class="icon">
                    <i class="las la-check"></i>
                </div>
                <div class="title fs-12">{{ translate('Received')}}</div>
            </div>
        </div>
    </div>
    <div class="card mt-4">
    ...
    <div class="row">
                            ...
                            @foreach ($order->orderDetails as $key => $orderDetail)
                                <tr>
                                    ...
                                    @php
                                        ...
                                        $last_refund_date = (new DateTime($orderDetail->created_at))->add(new DateInterval('P'.$no_of_max_day.'D'));
                                        $today_date = new DateTime();
                                    @endphp
                                    <td>
                                    @if (($orderDetail->product != null) && ($orderDetail->product->refundable != 0) && ($today_date <= $last_refund_date && ($orderDetail->delivery_status == 'delivered') &&($orderDetail->refund_request == null) ))
        ...
        <div class="col-lg-3">
            <div class="card mt-4">
                ...
                <div class="card-body pb-0">
                    <table class="table table-borderless">
                        <tbody>
							...
							<tr>
			            <th class="w-50 fw-600">{{ translate('Special Discount') }}</th>
						<td class="text-right">
                                    <span class="text-italic">{{ single_price($order->special_discount) }}</span>
                                </td>
			        </tr>
                            <tr>
                ...
                @endphp
				<tr>
			            <th class="w-50 fw-600">{{ translate('Paid') }}</th>
			            <td class="text-right">{{ single_price($paid) }}</td>
			        </tr>
					<tr>
			            <th class="w-50 fw-600">{{ translate('Due') }}</th>
			            <td class="text-right">{{ single_price($order->grand_total-$paid) }}</td>
			        </tr>
                        </tbody>
                    </table>
                </div>
