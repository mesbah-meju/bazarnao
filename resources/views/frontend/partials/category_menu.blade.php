...
<div class="aiz-category-menu bg-white rounded @if(Route::currentRouteName() == 'home') shadow-sm" @else shadow-lg" id="category-sidebar" @endif>
  @php
      ...
      $groupProducts = \App\Models\Product::where($conditions)
          ->where('is_group_product', 1)->get();

      $offerCount = $nonPaginateProducts->count() + $groupProducts->count();
  @endphp

  <ul class="list-unstyled categories no-scrollbar py-2 mb-0 text-left" >
  <a href="{{ route('offers') }}" style="font-size: 15px; color: #AE3C86!important; font-weight: bold; margin-top: 60px;">
      <span class="cat-name">Offers</span>
      <span class="offerCount">{{ $offerCount }}</span>
  </a>
</ul>
