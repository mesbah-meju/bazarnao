...
<style>
    .moving-text {
        ...
        animation: move-text 10s linear infinite;  /* 10s duration, linear motion, infinite loop */
    }
                        ...
                        <div class="text-left">
                            ...
                            @if($detailedProduct->is_group_product)
                                ...
                                @foreach($group_products as $group_product)
                                    <?php
                                        $product = App\Models\Product::find($group_product->product_id);
                                        $name = $product->name;
                                        $previous_price = $product->unit_price * $group_product->qty;

                                        $new_total_price = $group_product->price;
                                        $discount_amount = $previous_price - $new_total_price;

                                        $total_previous += $previous_price;
                                        $total_present += $new_total_price;
                                        $total_discount += $discount_amount;
                                    ?>
                                    <li class="list-group-item d-flex justify-content-between align-items-center py-3">
                                        <div class="d-flex flex-column">
                                            <span class="font-weight-bold">{{ $name }}</span>
                                            <span class="text-muted">Quantity: <span class="badge badge-primary badge-pill">{{ $group_product->qty }}</span></span>
                                        </div>
                                    </li>
                                @endforeach
                                </ul>
                            ...
                            <div class="row align-items-center mt-3">
                                <div class="col-auto">
                                    <div id="product-notice" class="product-notice">
                                        <small class="mr-2 opacity-50">{{ translate('Notice')}}: </small><br>
                                        <span class="moving-text">{{ strip_tags($detailedProduct->notice) }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            @else
                                <div class="row no-gutters mt-3">
                                    @if($detailedProduct->is_group_product)
                                    <div class="col-sm-2">
                                        <div class="opacity-50 my-2">{{ translate('Main Price')}}:</div>
                                    </div>
                                    <div class="col-sm-10">
                                        <div class="">
                                            <strong class="h2 fw-600 text-primary">
                                                TK {{ number_format($total_previous, 2)}}
                                            </strong>
                                        </div>
                                    </div>
                                    <div class="col-sm-2">
                                        <div class="opacity-50 my-2">{{ translate('Total Discount')}}:</div>
                                    </div>
                                    <div class="col-sm-10">
                                        <div class="">
                                            <strong class="h2 fw-600 text-primary">
                                                TK {{ number_format($total_discount, 2)}}
                                            </strong>
                                        </div>
                                    </div>
                                    @endif
                                    <div class="col-sm-2">
                                        @if($detailedProduct->is_group_product)
                                            <div class="opacity-50 my-2">{{ translate('New Price')}}:</div>
                                        @else
                                            <div class="opacity-50 my-2">{{ translate('Price')}}:</div>
                                        @endif
                                    </div>
                                        ...
                                        <div class="">
                                            <strong class="h2 fw-600 text-primary">
                                                <?php
                                                    ...
                                                    if($detailedProduct->is_group_product == 1){
                                                        $group_product = \App\Models\Group_product::where('group_product_id',$detailedProduct->id)->get();
                                                ...
                                                ?>
                                                @if($detailedProduct->is_group_product)
                                                    {{format_price($group_product_price)}}
                                                @else
                                                    {{ main_home_discounted_price($detailedProduct->id) }}
                                                @endif
                                            </strong>
                            ...
                            <form id="option-choice-form">
                                @csrf
                                <input type="hidden" name="id" id="prod_id" value="{{ $detailedProduct->id }}">
                                ...
                                <div class="row no-gutters">
                                    ...
                                    <div class="col-sm-10">
                                        @php
                                        if(in_array($detailedProduct->id,$cartIds)==true){
                                            $pkey = $keys[$detailedProduct->id];
                                            $procartqty = $cartqty[$detailedProduct->id];
                                            $showt = 'block';
                                            $showf = 'none';
                                        } elseif(in_array($detailedProduct->id,$cartIds)==false){
                                            $pkey = '';
                                            $procartqty = 0;
                                            $showf = 'block';
                                            $showt = 'none';
                                        ...
                                        <div class="product-quantity d-flex align-items-center">
                                            <div class="row no-gutters align-items-center aiz-plus-minus mr-3" style="width: 130px;">
                                                <button class="btn col-auto btn-icon btn-sm btn-circle btn-light" type="button" data-type="minus" data-field="quantity" data-key="{{ $pkey }}" data-id="{{ $detailedProduct->id }}">
                                                    <i class="las la-minus"></i>
                                                </button>
                                                <input id="product_{{ $detailedProduct->id }}" type="number" name="quantity" class="col border-0 text-center flex-grow-1 fs-16 input-number" placeholder="1" value="{{ $procartqty }}" min="0" max="100" readonly>
                                                <button class="btn col-auto btn-icon btn-sm btn-circle btn-light" type="button" data-type="plus" data-field="quantity" data-key="{{ $pkey }}" data-id="{{ $detailedProduct->id }}">
                            ...
                            <div class="mt-3">
                                @if ($detailedProduct->outofstock==0)
                                ...
                                <div class="p-4">
                                    ...
                                    @if(Auth::check())
                                        ...
                                        @foreach ($detailedProduct->orderDetails as $key => $orderDetail)
                                            @if($orderDetail->order != null && $orderDetail->order->user_id == Auth::user()->id && ($orderDetail->delivery_status == 'delivered' || $orderDetail->delivery_status == 'received') && \App\Models\Review::where('user_id', Auth::user()->id)->where('product_id', $detailedProduct->id)->first() == null)
                                                @php
                                                    $commentable = true;
                                                @endphp
                                            @endif
...
@section('script')
    <script type="text/javascript">
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (productNotice) {
                setTimeout(() => {
                    productNotice.classList.add('show');
