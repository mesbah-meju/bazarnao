    ...
    <div class="aiz-user-sidenav rounded overflow-hidden  c-scrollbar-light">
        <div class="px-4 text-center mb-4">
            ...
            @if(Auth::user()->user_type == 'customer')
                ...
                <h4 class="h5 fw-600">ID : {{ Auth::user()->customer->customer_id }}</h4>
        ...
        <div class="sidemnenu mb-3">
            <ul class="aiz-side-nav-list" data-toggle="aiz-side-menu">
                ...
                @endif
				<li class="aiz-side-nav-item">
                    <a href="{{ route('referral_link.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['referral_link.index'])}}">
                        <i class="la la-heart-o aiz-side-nav-icon"></i>
                        <span class="aiz-side-nav-text">{{ translate('Referral Link') }}</span>
                    </a>
                </li>
                <li class="aiz-side-nav-item">
        ...
        @if (App\Models\BusinessSetting::where('type', 'vendor_system_activation')->first()->value == 1 && Auth::user()->user_type == 'customer')
            <div>
          ...
          <div class="widget-balance pb-3 pt-1">
            <div class="text-center">
                ...
                <table class="table table-borderless">
                    ...
                    <tr>
                        @php
                            $orderDetails = App\Models\OrderDetail::where('seller_id', Auth::user()->id)->where('created_at', '>=', $days_ago_60)->where('created_at', '<=', $days_ago_30)->get();
                            $total = 0;
                            foreach ($orderDetails as $key => $orderDetail) {
                                if($orderDetail->order != null && $orderDetail->order->payment_status == 'paid'){
                                    $total += $orderDetail->price;
